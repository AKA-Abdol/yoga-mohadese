...
import { IShopCourseCard, ShopCourseStatus } from "../types";
...
const ShopCourseCard: React.FC<IShopCourseCard> = ({
  ...
  const [shopItemStatus, setShopItemStatus] = useState<ShopCourseStatus>();
  ...
  useEffect(() => {
    if (shopData) {
      ...
      const isItemInCard = cartData?.forEach((item) => {
        if (item.product.id === thisCourseCart?.id) return true;
      });
      if (isItemInCard) {
        setShopItemStatus("inCart");
      } else {
        setShopItemStatus("available");
      }
    }
  }, []);
          ...
          >
            ...
            {shopItem.hasAccess ? (
              ...
              <button
                onClick={() => {
                  setShopItemStatus("inCart");
                  onQuantityChange("add")(shopItem.id);
                }}
              ...
              <button
                onClick={() => {
                  setShopItemStatus("available");
                  onQuantityChange("delete")(shopItem.id);
                }}
